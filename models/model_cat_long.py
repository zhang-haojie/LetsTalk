...
class TransformerBlock(nn.Module):
    """
    A VDT tansformer block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
    def __init__(self, hidden_size, num_heads, context_dim=None, num_frames=16, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
        ...
        if context_dim is not None:
            ...
            self.cross_attn = AudioAttention(hidden_size, num_heads, context_dim)
    ...
    def forward(self, x, cond, motion, c, audio_semantic):
        ...
        x_cat = x_cat + attn
        # Feed Forward
        mlp = self.mlp(modulate(self.norm2(x_cat), shift_mlp, scale_mlp, self.num_frames))
        ...
        cond = x_cat[:, x_cat.size(1)//2:, ...] # >> End

        return x, cond, motion

    def _deprecated_forward(self, x, cond, motion, c, audio_semantic):
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
        T = self.num_frames
        initial_frames = motion.size(1)
        K, N, M = x.shape
        ...
        # Audio Attention
        x = x + self.cross_spatial(self.norm4(x), audio_semantic)
        ...
        x_motion_cat = torch.cat([x_motion, cond], dim=1)
        # Spatial Attention
        attn = self.attn(modulate(self.norm1(x_motion_cat), shift_msa, scale_msa, T+initial_frames))
        attn = rearrange(attn, '(b t) n m -> b (t n) m', b=B, t=T+initial_frames, n=2*N, m=M)
        attn = gate_msa.unsqueeze(1) * attn
        attn = rearrange(attn, 'b (t n) m -> (b t) n m', b=B, t=T+initial_frames, n=2*N, m=M)
        x_motion_cat = x_motion_cat + attn

        ## Feed Forward
        mlp = self.mlp(modulate(self.norm2(x_motion_cat), shift_mlp, scale_mlp, T+initial_frames))
        mlp = rearrange(mlp, '(b t) n m -> b (t n) m', b=B, t=T+initial_frames, n=2*N, m=M)
        mlp = gate_mlp.unsqueeze(1) * mlp
        mlp = rearrange(mlp, 'b (t n) m -> (b t) n m', b=B, t=T+initial_frames, n=2*N, m=M)
        x_motion_cat = x_motion_cat + mlp

        x_motion =  x_motion_cat[:, :x_motion_cat.size(1)//2, ...]
        cond = x_motion_cat[:, x_motion_cat.size(1)//2:, ...]
        x_motion = rearrange(x_motion, '(b t) n m -> b t n m', b=B, t=T+initial_frames, n=N, m=M)
        motion = x_motion[:, :initial_frames]
        x = x_motion[:, initial_frames:]
        x = rearrange(x, 'b t n m -> (b t) n m', b=B, t=T, n=N, m=M)
...
class VDT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            TransformerBlock(hidden_size, num_heads, context_dim=context_dim, num_frames=self.num_frames, mlp_ratio=mlp_ratio, attention_mode=attention_mode) for _ in range(depth)
        ])
...
def VDT_XL(**kwargs):
    return VDT(depth=28, hidden_size=1152, patch_size=1, num_heads=16, **kwargs)

def VDT_XL_2(**kwargs):
    return VDT(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)

def VDT_XL_4(**kwargs):
    return VDT(depth=28, hidden_size=1152, patch_size=4, num_heads=16, **kwargs)

def VDT_L(**kwargs):
    return VDT(depth=24, hidden_size=1024, patch_size=1, num_heads=16, **kwargs)

def VDT_L_2(**kwargs):
    return VDT(depth=24, hidden_size=1024, patch_size=2, num_heads=16, **kwargs)

def VDT_L_4(**kwargs):
    return VDT(depth=24, hidden_size=1024, patch_size=4, num_heads=16, **kwargs)

def VDT_B_2(**kwargs):
    return VDT(depth=12, hidden_size=768, patch_size=2, num_heads=12, **kwargs)

def VDT_B(**kwargs):
    return VDT(depth=12, hidden_size=768, patch_size=1, num_heads=12, **kwargs)

def VDT_B_4(**kwargs):
    return VDT(depth=12, hidden_size=768, patch_size=4, num_heads=12, **kwargs)

def VDT_S(**kwargs):
    return VDT(depth=12, hidden_size=384, patch_size=1, num_heads=6, **kwargs)

def VDT_S_2(**kwargs):
    return VDT(depth=12, hidden_size=384, patch_size=2, num_heads=6, **kwargs)

def VDT_S_4(**kwargs):
    return VDT(depth=12, hidden_size=384, patch_size=4, num_heads=6, **kwargs)


VDTcatlong_models = {
    'VDTcatlong-XL': VDT_XL, 'VDTcatlong-XL/2': VDT_XL_2,  'VDTcatlong-XL/4': VDT_XL_4,
    'VDTcatlong-L':  VDT_L, 'VDTcatlong-L/2':  VDT_L_2,   'VDTcatlong-L/4':  VDT_L_4,
    'VDTcatlong-B':  VDT_B, 'VDTcatlong-B/2':  VDT_B_2,   'VDTcatlong-B/4':  VDT_B_4,
    'VDTcatlong-S':  VDT_S, 'VDTcatlong-S/2':  VDT_S_2,   'VDTcatlong-S/4':  VDT_S_4,
}
