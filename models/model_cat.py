...
class AudioAttention(nn.Module):
    def __init__(self, dim, num_heads=8, context_dim=None, qkv_bias=False, attn_drop=0., proj_drop=0., use_lora=False, attention_mode='math'):
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'
        self.num_heads = num_heads
        head_dim = dim // num_heads
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, cond, c, audio_semantic):
...
class VDT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            TransformerBlock(hidden_size, num_heads, context_dim=context_dim, num_frames=num_frames, mlp_ratio=mlp_ratio, attention_mode=attention_mode) for _ in range(depth)
        ])

        self.final_layer = FinalLayer(hidden_size, patch_size, self.out_channels, num_frames)
    ...
    def forward(self,
        ...
        # Temporal embed
        x = rearrange(x, '(b t) n m -> (b n) t m',b=batches,t=frames)
        ## Resizing time embeddings in case they don't match
        x = x + self.temp_embed
        x = rearrange(x, '(b n) t m -> (b t) n m',b=batches,t=frames)

        audio_semantic = rearrange(audio_embed, 'b f n k -> (b f) n k')

        t = self.t_embedder(t)   # [5, 384]
        if y is not None:
            c = t + y
        else:
            c = t
        ...
        for i, block in enumerate(self.blocks):
            if self.gradient_checkpointing:
                x, cond = checkpoint(block, x, cond, c, audio_semantic)
            else:
                x, cond = block(x, cond, c, audio_semantic)
...
def VDT_XL_8(**kwargs):
    return VDT(depth=28, hidden_size=1152, patch_size=8, num_heads=16, **kwargs)
...
def VDT_L_8(**kwargs):
    return VDT(depth=24, hidden_size=1024, patch_size=8, num_heads=16, **kwargs)
...
def VDT_B_8(**kwargs):
    return VDT(depth=12, hidden_size=768, patch_size=8, num_heads=12, **kwargs)
...
def VDT_S_8(**kwargs):
    return VDT(depth=12, hidden_size=384, patch_size=8, num_heads=6, **kwargs)


VDTcat_models = {
    'VDTcat-XL/2': VDT_XL_2,  'VDTcat-XL/4': VDT_XL_4,  'VDTcat-XL/8': VDT_XL_8,
    'VDTcat-L/2':  VDT_L_2,   'VDTcat-L/4':  VDT_L_4,   'VDTcat-L/8':  VDT_L_8,
    'VDTcat-B/2':  VDT_B_2,   'VDTcat-B/4':  VDT_B_4,   'VDTcat-B/8':  VDT_B_8,
    'VDTcat-S/2':  VDT_S_2,   'VDTcat-S/4':  VDT_S_4,   'VDTcat-S/8':  VDT_S_8,
}
